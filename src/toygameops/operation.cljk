(ns toygameops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (ToyGameRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the ToyGameRetailGovernor (:govern)
  ...
  (:require [langgraph.graph :as g]
            ...
            [toygameops.governor :as governor]
            [toygameops.phase :as phase]
...
(defn build
  ...
  `toygameops.store/Store`).
  opts:
    :advisor      -- a `toygameops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ToyGameRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; ToyGameRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
