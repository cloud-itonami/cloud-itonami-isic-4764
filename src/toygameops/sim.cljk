(ns toygameops.sim
  ...
  into the permanently-excluded recall-compliance/age-grading-safety
  finalization scope."
...
(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :toy-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :toy-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 27 :returns 2 :stock-count-delta -29}} coordinator-phase-1)]
      (println r)
      (println "-- human toy store coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 15 :returns 1 :stock-count-delta -16}} coordinator-phase-3))
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "holiday construction-set pallet" :quantity 40 :estimated-cost 2400.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human toy store coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-safety-concern :store-id "store-1"
                                 :patch {:concern "small magnetic parts detaching from a construction set on the under-3 endcap, possible age-grading label mismatch" :confidence 0.92}} coordinator-phase-3)]
      (println r)
      (println "-- human toy store coordinator reviews & approves --")
    ...
    (println "\n== log-sales-record store-1, advisor drifts into recall/age-grading finalization scope -> HARD hold, permanent ==")
