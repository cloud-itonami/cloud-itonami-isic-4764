(ns toygameops.governor-test
  "Pure unit tests of `toygameops.governor/check` against hand-built
  ...
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [toygameops.governor :as gov]
            [toygameops.store :as store]))

(def store-1 {:store-id "store-1" :name "Riverside Games & Toys" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Toy Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Northgate Toy & Game Distribution" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Toy Broker Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine toy store coordination"
...
(deftest recall-finalization-content-is-hard-and-permanent
  (testing "a proposal whose rationale touches directly finalizing a recall is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "finalized the recall for the affected SKU and closed out the case"
                          :confidence 0.99)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest recall-compliance-certification-content-is-hard
  (testing "a proposal touching certifying recall compliance is HARD-blocked, same as finalizing the recall itself"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "certified recall compliance for the entire batch before it left the stockroom"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest age-rating-approval-content-is-hard
  (testing "a proposal touching approving the age rating is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :schedule-staffing-operation "store-1")
                          :summary "floor staff should approve the age rating on the repackaged sets before restocking")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest age-grade-certification-content-is-hard
  (testing "a proposal touching certifying the age grade is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "certified the age grade on the imported assortment at receiving")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-safety-concern-is-not-scope-excluded
  (testing "flagging observed choking-hazard/recall-suspicion/age-grading-mismatch concerns as a SAFETY CONCERN (not a finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-safety-concern "store-1")
                         :value {:concern "small magnetic parts detaching from a construction set, possible recall-affected batch, age-grading label appears mismatched"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (choking hazard/suspected recall/age-grading mismatch) is exactly what this op exists to surface"))))
...
;; "recall" or "age grade"), which then accidentally matches inside the
    ...
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "board-game restock" :estimated-cost 380.0 :vendor-id "vendor-1"}
                      {})
